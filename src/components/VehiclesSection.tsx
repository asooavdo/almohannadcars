...
const VehicleCard = ({ vehicle, index }: { vehicle: Vehicle; index: number }) => {
  const whatsappMessage = encodeURIComponent(
    `مرحباً، أنا مهتم بسيارة ${vehicle.name} ${vehicle.year}. أرجو التواصل معي للمزيد من التفاصيل.`
  );

  return (
    <div 
      className="group luxury-card overflow-hidden hover-lift opacity-0 animate-fade-up"
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-background/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <a 
            href={`https://wa.me/962797055388?text=${whatsappMessage}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-md font-semibold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
          >
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          ...
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 bg-secondary hover:bg-primary text-secondary-foreground hover:text-primary-foreground px-4 py-2 rounded-md transition-all duration-300"
          >
...
const VehiclesSection = () => {
  return (
    <section id="vehicles" className="py-24 bg-gradient-to-b from-background via-card to-background">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="text-center mt-12">
          <a 
            href="https://wa.me/962797055388?text=مرحباً، أرغب في الاطلاع على جميع السيارات المتوفرة لديكم"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 border border-primary text-primary px-8 py-4 rounded-md font-semibold hover:bg-primary hover:text-primary-foreground transition-all duration-300"
          >
